...
export default class OrderForm extends React.Component {
    ...
    handleProductChange = (e) => {
        this.setState({ product: e.target.value, inputError: "" });
    }
    ...
    validateUserInput() {
        ...
        if (this.state.company.length < 3) {
            this.setState({ error: "Company name needs to be at least 3 characters long " })
            return false
        }
        if (this.state.name.length < 3) {
            this.setState({ error: "Name needs to be at least 3 characters long" })
            return false
        }
        if (this.state.email.length < 3) {
            this.setState({ error: "Email needs to be at least 3 characters long." })
            return false
        }
        if (this.state.phone.length !== 10) {
            this.setState({ error: "Phone needs to be 10 characters in length." })
            return false
        }
        if (this.state.product.length <= 0) {
            this.setState({ error: "Product needs to be selected" })
            return false
        }
        if (this.state.quantity.length <= 0) {
            this.setState({ error: "Quantity needs to be greater than zero." })
            return false
        }
    ...
    handleSubmit = async (e) => {
        ...
        axios.post('http://localhost:8080/email/', params)
            .then(res => {
                    ...
                    <div className="form-style-3">
                        <form>
                            <fieldset>
                                ...
                                <label className="required">{this.state.error}</label>
